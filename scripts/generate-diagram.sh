...
Options:
    ...
    -s, --style STYLE      Deprecated compatibility option; ignored
...
Examples:
    $0 -t architecture -o ./output/arch.svg
    $0 -t class -w 2400
    $0 -t sequence
USAGE
    exit 0
...
if [ "${STYLE:-}" != "default" ] && [ "${STYLE:-}" != "6" ]; then
    echo -e "${YELLOW}Warning: style selection is deprecated; using the built-in default visual system${NC}"
fi

# Determine output path
if [ -z "${OUTPUT_PATH:-}" ]; then
    BASENAME="${TYPE}-default"
...
echo -e "${BLUE}Generating ${TYPE} diagram (default visual system)...${NC}"
echo "Output: $SVG_FILE"

# Load visual reference
SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
STYLE_FILE="${SKILL_DIR}/references/default-visual-system.md"

if [ -z "${STYLE_FILE:-}" ] || [ ! -f "$STYLE_FILE" ]; then
    echo -e "${RED}Error: Visual reference file not found: ${STYLE_FILE}${NC}"
    exit 1
fi
