...
Do not pass `@hw-yue/svg-tech-graph` directly to `skills add`, because the CLI expects a GitHub or local repository source.
...
```
- Tests the retained default-visual-system fixture set
- Validates generated SVGs
...
Only one style is retained in this skill:

- **Default visual system**
- Background: warm cream `#f8f6f3`
- Best for: product docs, architecture explainers, polished presentations

Load `references/default-visual-system.md` for exact color tokens and SVG patterns.

## Style Selection

**Default and only option**: the built-in default visual system.
